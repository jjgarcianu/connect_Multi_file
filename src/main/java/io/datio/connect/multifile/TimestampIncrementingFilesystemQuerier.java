 ...
 * </p>
 */
public class TimestampIncrementingFilesystemQuerier extends FilesystemQuerier {
  private static final Logger log = LoggerFactory.getLogger(TimestampIncrementingFilesystemQuerier.class);
  ...
  public TimestampIncrementingFilesystemQuerier(QueryMode mode, String name, String topicPrefix,
  ...
  protected void createFileRegex(FilesysAdaptor db) throws FilesysException {
    ...
    if (incrementingColumn != null && incrementingColumn.isEmpty()) {
      incrementingColumn = FilesystemUtils.getAutoincrementColumn(db, name);
    }
    ...
    switch (mode) {
      case TABLE:
        ...
        builder.append(FilesystemUtils.quoteString(name, quoteString));
        break;
      case QUERY:
    ...
    if (incrementingColumn != null && timestampColumn != null) {
      ...
      builder.append(FilesystemUtils.quoteString(timestampColumn, quoteString));
      builder.append(" < CURRENT_TIMESTAMP AND ((");
      builder.append(FilesystemUtils.quoteString(timestampColumn, quoteString));
      builder.append(" = ? AND ");
      builder.append(FilesystemUtils.quoteString(incrementingColumn, quoteString));
      builder.append(" > ?");
      builder.append(") OR ");
      builder.append(FilesystemUtils.quoteString(timestampColumn, quoteString));
      builder.append(" > ?)");
  ...
  protected FileRegexResultset executeQuery() throws FilesysException {
    if (incrementingColumn != null && timestampColumn != null) {
      Date ts = new Date(timestampOffset == null ? 0 : timestampOffset);
  ...
  public SourceRecord extractRecord() throws FilesysException {
    ...
    if (incrementingColumn != null) {
      ...
      offset.put(FilesystemSourceTask.INCREMENTING_FIELD, id);
    }
    ...
    if (timestampColumn != null) {
      ...
      offset.put(FilesystemSourceTask.TIMESTAMP_FIELD, timestampOffset);
    }
