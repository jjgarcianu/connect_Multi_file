...
/**
 * DataConverter handles translating flat file schemas to Kafka Connect schemas and file record (line) data to Kafka
 * Connect records.
 */
public class DataConverter {
  ...
  public static Schema convertSchema(String tableName, FilesysEntityMetadata metadata)
  ...
  public static Struct convertRecord(Schema schema, FileRegexResultset resultSet)
      throws FilesysException {
    FilesysEntityMetadata metadata = resultSet.getMetaData();
  ...
  private static void addFieldSchema(FilesysEntityMetadata metadata, int col,
    ...
    int dataType = metadata.getColumnType(col);
    boolean optional = false;
    if (metadata.isNullable(col) == FileRegexResultset.columnNullable ||
        metadata.isNullable(col) == FileRegexResultset.columnNullableUnknown) {
      optional = true;
    }
    switch (dataType) {
      case Types.NULL: {
        log.warn("Flat File type {} not currently supported", dataType);
        break;
      }
  ...
  private static void convertFieldValue(FileRegexResultset resultSet, int col, int colType,
                                        Struct struct, String fieldName)
      throws FilesysException, IOException {
